...
const round = (n: number, d = 1) => (Number.isFinite(n) ? Number(n.toFixed(d)).toString() : "—");
...
export const calculators: Calculator[] = [
  ...
  },
  {
    ...
    compute: ({ sbp, dbp }) => {
      ...
      const label =
        map < 65 ? "Low — organ perfusion risk" : map > 110 ? "Elevated" : "Adequate perfusion";
      return {
        value: round(map, 0),
        unit: "mmHg",
        interpretation: [{ label, tone }],
